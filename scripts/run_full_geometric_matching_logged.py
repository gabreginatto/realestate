#!/usr/bin/env python3
"""
Run full geometric matching with detailed progress logging.

Logs to geometric_progress.log for monitoring.
"""
...
class ProgressLogger:
    ...
    def __init__(self, log_file="geometric_progress.log"):
        self.log_file = log_file
        self.start_time = time.time()

        # Initialize log file
        with open(self.log_file, 'w') as f:
            f.write(f"=== GEOMETRIC MATCHING PROGRESS LOG ===\n")
...
def main():
    console.print("\n[bold cyan]🔍 Full Geometric Matching: Viva ↔ Coelho (WITH LOGGING)[/bold cyan]\n")
    ...
    # Initialize logger
    logger = ProgressLogger("geometric_progress.log")
    ...
    logger.log(f"Total pairs: {total_pairs:,}")
    logger.log("")
    ...
    console.print(f"  • Progress log: geometric_progress.log\n")
        ...
        for coelho_id in coelho_listings:
            ...
            for viva_id in viva_listings:
                ...
                if result["is_match"]:
                    ...
                    logger.log_pair(coelho_id, viva_id, result, pair_time)

                # Log progress every 10 pairs
                if comparison_count % 10 == 0:
                    avg_time = np.mean(pair_times)
                    logger.log_progress(comparison_count, total_pairs, match_count, avg_time)

                # Log every 100 pairs with more detail
                if comparison_count % 100 == 0:
                    elapsed = time.time() - start_time
                    avg_time = np.mean(pair_times)
                    logger.log("")
                    logger.log(f"CHECKPOINT: {comparison_count}/{total_pairs} pairs")
                    ...
                    logger.log(f"  Recent 100: min={min(pair_times[-100:])*1000:.0f}ms, max={max(pair_times[-100:])*1000:.0f}ms")
                    logger.log("")
    ...
    logger.log(f"Total pairs: {total_pairs:,}")
    logger.log(f"Matches found: {match_count}")
    logger.log(f"Match rate: {match_count/total_pairs*100:.2f}%")
    logger.log(f"Total time: {elapsed_time/60:.1f} minutes")
    logger.log(f"Avg time/pair: {(elapsed_time * 1000) / total_pairs:.0f}ms")
    logger.log("="*60)
    ...
    console.print(f"Progress log: [cyan]geometric_progress.log[/cyan]\n")
