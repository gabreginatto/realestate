#!/usr/bin/env python3
"""
FAST Price-Filtered Geometric Matching

Optimizations applied:
1. Parallel processing (multiprocessing)
2. Reduced ORB features (2000 → 1000)
3. Fewer images per listing (12 → 6)
4. Lower min inliers (30 → 25)
5. Added inlier ratio check (≥0.20)
6. Image downscaling for faster processing
7. Aggressive early termination

Expected speedup: 4-6x faster (~10-15 minutes instead of 50)
"""
...
class ProgressLogger:
    ...
    def __init__(self, log_file="geometric_progress_fast.log"):
        self.log_file = log_file
        self.start_time = time.time()

        # Initialize log file
        with open(self.log_file, 'w') as f:
            f.write(f"=== FAST PRICE-FILTERED GEOMETRIC MATCHING LOG ===\n")
...
class FastGeometricMatcher:
    ...
    def __init__(self, num_features=1000, min_inliers=25, min_inlier_ratio=0.20, ransac_threshold=5.0):
    ...
    def match_listings(self, listing1_dir: str, listing2_dir: str, max_images: int = 6) -> Dict:
        """
        ...
        for img1 in images1:
            for img2 in images2:
                ...
                # Aggressive early termination - stop as soon as we find a good match
...
def main():
    console.print("\n[bold cyan]🚀 FAST Price-Filtered Geometric Matching[/bold cyan]\n")
    ...
    output_file = "geometric_matches_fast.json"

    # Initialize logger
    logger = ProgressLogger("geometric_progress_fast.log")
    ...
    console.print("[bold yellow]⚡ OPTIMIZATIONS:[/bold yellow]")
    console.print("  • Parallel processing: Using all CPU cores")
    console.print("  • ORB features: 1000 (was 2000)")
    console.print("  • Max images: 6 per listing (was 12)")
    console.print("  • Min inliers: 25 (was 30)")
    console.print("  • Min inlier ratio: 0.20")
    console.print("  • Image downscaling: 800px max dimension")
    console.print("  • Aggressive early termination\n")
    ...
    output_data = {
        "metadata": {
            "approach": "Fast price-filtered geometric matching (parallel + optimized)",
    ...
    # Summary
    console.print(f"\n[bold green]✓ Fast geometric matching complete![/bold green]\n")
    ...
    console.print(f"  • Speedup: ~{51.6 / (elapsed_time/60):.1f}x faster than previous run\n")
